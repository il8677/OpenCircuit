...
Chunk* Debug::getSubcircuitChunk(Chunk* c, int i) {
	return c->subcircuits[i].getChunk();
}

std::string Debug::getSubcircuitName(Chunk* c, int i) {
	return c->subcircuits[i].getName();
}

void Debug::drawUpdateOverlay(SFMLWindow& w, Chunk* c, int uo) {
